...
const GestionArchivosOficiales = () => {
  ...
  const filtrarDocumentos = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(doc => 
        doc.codigoValidacion?.toLowerCase().includes(term) ||
        `${doc.paciente?.nombres} ${doc.paciente?.apellido_paterno} ${doc.paciente?.apellido_materno}`.toLowerCase().includes(term) ||
 ...
 const handleInputChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Validar fechas cuando cambie fechaEmision o fechaVigencia
 ...
 const formatearFecha = (fecha) => {
    if (!fecha) return '-';
    // Extraer solo la parte de la fecha (YYYY-MM-DD) sin problemas de zona horaria
    ...
    return fechaCorrecta.toLocaleDateString('es-PE', { 
      ...
      month: 'long', 
      day: 'numeric',
      timeZone: 'UTC' // CRÍTICO: Usar UTC para evitar conversiones de zona horaria
    });
  };
              ...
              <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 2 }}>
                ...
                <Box>
                  ...
                  <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.8)' }}>
                    Sistema de gestión documental con validación única
                  </Typography>
                </Box>
              </Stack>
                      ...
                      <Table>
                        <TableHead>
                          <TableRow sx={{ background: 'linear-gradient(to right, #f8fafc, #f1f5f9)' }}>
                            ...
                            <TableCell sx={{ fontWeight: 700, color: '#475569' }}>Paciente</TableCell>
                            ...
                            .map((doc) => (
                              ...
                              >
                                ...
                                <TableCell sx={{ fontWeight: 500 }}>
                                  {`${doc.paciente?.nombres || ''} ${doc.paciente?.apellido_paterno || ''}`}
                                </TableCell>
                                <TableCell>
                                  <Chip 
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Autocomplete
                        ...
                        getOptionLabel={(option) => 
                          `${option.nombres} ${option.apellido_paterno} ${option.apellido_materno} - ${option.numero_documento}`
                        }
                        value={pacienteSeleccionado}
                        onChange={(e, newValue) => {
                          setPacienteSeleccionado(newValue);
                          setFormData(prev => ({ ...prev, pacienteId: newValue?.id || '' }));
                        }}
                        renderInput={(params) => (
                          <TextField
                            {...params}
                  ...
                  <Grid item xs={12}>
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Autocomplete
                        options={terapeutas}
                        getOptionLabel={(option) => 
                          `${option.nombres} ${option.apellidos}${option.especialidad ? ` - ${option.especialidad}` : ''}`
                        }
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="body1" sx={{ fontWeight: 600, color: '#1e293b', mt: 0.5 }}>
                    {`${documentoSeleccionado.paciente?.nombres || ''} ${documentoSeleccionado.paciente?.apellido_paterno || ''} ${documentoSeleccionado.paciente?.apellido_materno || ''}`}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="body1" sx={{ fontWeight: 600, color: '#1e293b', mt: 0.5 }}>
{formatearFecha(documentoSeleccionado.fechaEmision)}      </Typography> 
                </Grid>
